/* ============================================================
   ...
   contract states as "returns `[]` when the declared version is at
   least the inferred level and a diagnostic carrying the engine's
   own `reasons` when it is not".
   ============================================================ */
...
describe("AC-1: a declared bump smaller than the inferred one is refused", () => {
  it("AC-1 refuses a major change the author declared as a minor, naming the engine's reasons", async () => {
    ...
    const ds = asDiagnostics(fn("1.0.0", "1.1.0", inferred), WHERE);

    expect(ds.length, "1.1.0 on 1.0.0 is a minor bump and the change requires a major").toBeGreaterThan(0);
  ...
  it("AC-1 refuses at error severity, because a refusal a caller may ignore is not a refusal", async () => {
    ...
    // B-03 puts data and diagnostics together at 200, so `hasErrors` is the only thing
    // that distinguishes "this release is refused" from "here is a note about it".
  ...
  it("AC-1 accepts the same change once the author declares the major", async () => {
    ...
    expect(asDiagnostics(fn("1.0.0", "2.0.0", inferred), WHERE)).toEqual([]);
  });
  ...
  it("AC-1 holds every consecutive pair of a chain, and refuses only the step that is short", async () => {
    ...
    const chain = [
      card({ version: "1.0.0" }),
      card({ version: "1.1.0", inputs: [{ name: "task", type: "text" }, { name: "hint", type: "text", required: false }] }),
      card({ version: "1.2.0", inputs: [{ name: "task", type: "text" }, { name: "hint", type: "text", required: false }], outputs: [] }),
      card({ version: "2.0.0", inputs: [{ name: "task", type: "text" }, { name: "hint", type: "text", required: false }], outputs: [], name: "Solver II" }),
    ];

    const refused = chain.slice(1).map((next, i) => {
      const previous = chain[i];
      const ds = asDiagnostics(fn(previous.version, next.version, inferBump(previous, next)), WHERE);
      return { step: `${previous.version} → ${next.version}`, count: ds.length };
    });

    expect(refused).toEqual([
      { step: "1.0.0 → 1.1.0", count: 0 },
      { step: "1.1.0 → 1.2.0", count: expect.any(Number) },
      { step: "1.2.0 → 2.0.0", count: 0 },
    ]);
    expect(refused[1].count, "the step that removed an output while declaring a minor").toBeGreaterThan(0);
  });
});
...
describe("checkDeclaredBump: what `at least the inferred level` means", () => {
  it("accepts a declaration stronger than the change requires", async () => {
    const fn = await checkDeclaredBump();
    expect(asDiagnostics(fn("1.0.0", "2.0.0", analysis("patch", ["`notes` changed"])), WHERE)).toEqual([]);
  });

  it("refuses a patch declared against an inferred minor", async () => {
    const fn = await checkDeclaredBump();
    const ds = asDiagnostics(fn("1.0.0", "1.0.1", analysis("minor", ["output `draft` was added"])), WHERE);
  ...
  it("refuses a version that did not move at all", async () => {
    const fn = await checkDeclaredBump();
    const ds = asDiagnostics(fn("1.0.0", "1.0.0", analysis("patch", ["`notes` changed"])), WHERE);
    expect(ds.length, "an unchanged version declares no bump, and the change requires a patch").toBeGreaterThan(0);
  });

  it("refuses a version that went backwards", async () => {
    const fn = await checkDeclaredBump();
    const ds = asDiagnostics(fn("2.0.0", "1.0.0", analysis("patch", ["`notes` changed"])), WHERE);
    expect(ds.length).toBeGreaterThan(0);
  });
  ...
  it("accepts anything when the engine inferred `none`", async () => {
    ...
    expect(asDiagnostics(fn("1.0.0", "1.0.1", analysis("none")), WHERE)).toEqual([]);
    expect(asDiagnostics(fn("1.0.0", "1.0.0", analysis("none")), WHERE)).toEqual([]);
  });
  ...
  it("refuses a major change even when the engine handed over no reasons", async () => {
    ...
    const ds = asDiagnostics(fn("1.0.0", "1.1.0", analysis("major")), WHERE);
    expect(ds.length).toBeGreaterThan(0);
  });
});
...
describe("checkDeclaredBump: malformed and boundary versions", () => {
  ...
  it.each(unparseable)("refuses a major change declared as %j, which is not a semver", async (declared) => {
    const fn = await checkDeclaredBump();
    const ds = asDiagnostics(fn("1.0.0", declared, analysis("major", ["output `answer` was removed"])), WHERE);
    expect(
      ds.length,
  ...
  it.each(unparseable)("refuses a major change published on top of %j, which is not a semver", async (previous) => {
    const fn = await checkDeclaredBump();
    const ds = asDiagnostics(fn(previous, "2.0.0", analysis("major", ["output `answer` was removed"])), WHERE);
    expect(
      ds.length,
  ...
  it("refuses when both versions are the empty string", async () => {
    const fn = await checkDeclaredBump();
    const ds = asDiagnostics(fn("", "", analysis("major", ["output `answer` was removed"])), WHERE);
    expect(ds.length).toBeGreaterThan(0);
  });

  it("accepts the smallest possible patch, 0.0.0 → 0.0.1", async () => {
    const fn = await checkDeclaredBump();
    expect(asDiagnostics(fn("0.0.0", "0.0.1", analysis("patch", ["`notes` changed"])), WHERE)).toEqual([]);
  });

  it("accepts 0.9.9 → 0.10.0 as a minor, which a string comparison would call a downgrade", async () => {
    const fn = await checkDeclaredBump();
    expect(asDiagnostics(fn("0.9.9", "0.10.0", analysis("minor", ["output `draft` was added"])), WHERE)).toEqual([]);
  });

  it("accepts a major across a boundary no 32-bit counter survives", async () => {
    const fn = await checkDeclaredBump();
    expect(
      asDiagnostics(fn("4294967295.0.0", "4294967296.0.0", analysis("major", ["output `answer` was removed"])), WHERE),
    ).toEqual([]);
  });
  ...
  it("refuses a major when the author only promoted a prerelease", async () => {
    ...
    const ds = asDiagnostics(fn("1.0.0-rc.1", "1.0.0", analysis("major", ["output `answer` was removed"])), WHERE);
    expect(ds.length).toBeGreaterThan(0);
  });
  ...
  it("accepts a major declared as a prerelease of the next major", async () => {
    const fn = await checkDeclaredBump();
    expect(
      asDiagnostics(fn("1.0.0", "2.0.0-rc.1", analysis("major", ["output `answer` was removed"])), WHERE),
    ).toEqual([]);
  });
});
...
describe("checkDeclaredBump: unreadable input is never a silent acceptance", () => {
  it.each([
    ...
    const fn = await checkDeclaredBump();
    neverSilentlyAccepts(() => fn("1.0.0", "1.0.1", inferred), WHERE);
  });
  ...
  it.each([
    ...
    const fn = await checkDeclaredBump();
    neverSilentlyAccepts(() => fn("1.0.0", declared, analysis("major", ["output `answer` was removed"])), WHERE);
  });
});
...
describe("checkDeclaredBump: unicode, purity and repetition", () => {
  it("carries a reason through verbatim, whatever is in it", async () => {
    ...
    const ds = asDiagnostics(fn("1.0.0", "1.0.1", analysis("major", reasons)), WHERE);
    const text = ds.map(diagnosticText).join("\n");
  ...
  it("does not touch the analysis it was handed", async () => {
    ...
    expect(() => fn("1.0.0", "1.1.0", inferred)).not.toThrow();
  ...
  it("gives every caller its own array, so one caller cannot poison the next", async () => {
    const fn = await checkDeclaredBump();
    const first = asDiagnostics(fn("1.0.0", "2.0.0", analysis("major", ["output `answer` was removed"])), WHERE);
    ...
    const second = asDiagnostics(fn("1.0.0", "2.0.0", analysis("major", ["output `answer` was removed"])), WHERE);
  ...
  it("answers a repeated call identically", async () => {
    const fn = await checkDeclaredBump();
    const inferred = inferBump(PUBLISHED_CARD, BREAKING_EDIT);
    const runs = Array.from({ length: 5 }, () => asDiagnostics(fn("1.0.0", "1.1.0", inferred), WHERE));
