...
describe("the control: what the archive credits", () => {
   ...
   *
   * The cards are asserted in the same cell because the pair is what carries the rule, and
   * the rule changed: D-250-18 kept a card's `author` as the person who wrote the document,
   * against a manifest's, which is the account it is listed under. The archive has no such
   * person. Its cards were generated to populate the registry, and the six persona names
   * they carried said otherwise, so the owner collapsed them onto the account that generated
   * them. The distinction D-250-18 protects is still real for a card somebody writes; it had
   * nothing to protect here.
   */
  it("credits every manifest and every generated card to the registry handle", () => {
...
describe("ownership and authorship name the same handle", () => {
   ...
   * show the import carried the file's own line through untouched rather than stamping
   * anything over it. That the line now names the publisher is the archive's own fact, not
   * the import's doing: these cards were generated by that account.
   */
  it(
    ...
    async () => {
      ...
      /* Still read back out of the stored bytes rather than assumed, which is what the cell
         is for: the import must carry the file through untouched, whatever the line says.
         What it says is now the account that generated these cards. */
