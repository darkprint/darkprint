...
describe("AC4 — clone by digest writes the server's export", () => {
  ...
  it("resolves a version through provenance and lands the same bytes as the digest clone", async () => {
    ...
    expect(readFileSync(join(byVersion, "topology.dot"), "utf8")).toBe(
      readFileSync(join(SOURCE, "topology.dot"), "utf8"),
    );
  });
...
describe("AC2 — bump refuses a declaration below the inferred one, naming the reasons", () => {
  ...
  function repinned(): string {
    ...
    for (const file of files) {
      ...
      // `SOURCE` is `public/bundles/frontline-triage`, regenerated by `npm run prebuild`
      // and not by this task — it still carries the topology under its pre-rename name
      // until that regeneration runs. This is the LOCAL working copy `bump` reads through
      // `readBundleDirectory`, which now looks for `topology.dot`, so the copy is written
      // under the current name rather than verbatim under the stale one.
      const destName = file === "blueprint.dot" ? "topology.dot" : file;
...
describe("runCli", () => {
  ...
  it("renders a CliError's message and nothing else — no stack, no endpoint", async () => {
    ...
    expect(printed).toContain("topology.dot");
