// forked from design-sync lib/source-kit.mjs - 'ui' dropped from GENERIC_DIR
...
export async function resolvePackage(ctx) {
  ...
  if (srcRoot) {
    for (const c of components) {
      ...
      // Fork note: dropping the segment that repeats the component's own name is
      // right for a `Button/Button.tsx` layout, but here the group folder IS the
      // name for `hero/Hero.tsx` — filtering it left the site's Hero in 'general'.
      // Falling back to the last non-generic segment keeps that case grouped and
      // cannot change any path where the first filter still yields something.
      const segs = slash(relative(srcRoot, dirname(hit))).split('/').filter(Boolean);
      const notGeneric = segs.filter((s) => !GENERIC_DIR.has(s.toLowerCase()));
      c.group = slug(
        notGeneric.filter((s) => s.toLowerCase() !== c.name.toLowerCase()).at(-1)
        || notGeneric.at(-1)
