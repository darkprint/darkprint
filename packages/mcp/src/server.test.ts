/* ============================================================
   The stdio transport, driven rather than read

   Everything goes through the real `serve` loop over a real pipe,
   with only `fetch` stubbed. A test calling the renderer directly
   would assert the package agrees with itself and stay green the
   day `tools/call` starts rendering a refusal as an RPC error,
   which is the one mistake that makes a rate limit unreadable to
   the model while every string in it stays correct.
   ============================================================ */
...
/** The registry's published 429 key set, byte for byte as `rateLimited()` emits it. */
const RATE_LIMITED = {
  ...
  instance: "/api/mcp/blueprints/find",
  limit: 600,
  remaining: 0,
...
const FIND = {
  ...
  id: 2,
  method: "tools/call",
  params: { name: "find_blueprints", arguments: { task: "x" } },
};
...
describe("the MCP transport speaks the protocol", () => {
  ...
  it("lists the seven tools and nothing else", async () => {
    ...
    /* An exact set and not a superset: a tool nobody defined is a capability nobody
       granted, and a missing one is a tool `/mcp` advertises. */
...
describe("an unkeyed client is limited and told so in a form an agent can act on", () => {
  it("renders the 429 as a TOOL RESULT, which is the only place the model reads", async () => {
    const [, call] = await session([INITIALIZE, FIND], answering(problem429(RATE_LIMITED)));

    /* NOT `error`. A JSON-RPC error is a transport fault the host shows the user and the
       model never sees, so a refusal rendered there is opaque to exactly the audience that
       has to act on it. */
  ...
  it("names the ceiling, the reset instant and the key affordance", async () => {
    ...
    /* `about ... from now` can only be produced by parsing `resetAt`; the detail sentence
       carries the instant too, so this is the assertion that measures the member. */
    ...
    expect(text, "an opaque refusal is the failure mode this exists to exclude").not.toBe("429");
  });
  ...
  it("reports the reset instant from a 429 that carries no `detail` at all", async () => {
    ...
    const [, call] = await session([INITIALIZE, FIND], answering(problem429(membersOnly)));
    const text = (call?.result as { content: { text: string }[] }).content[0].text;
  ...
  it("says nothing about keys when the registry does not offer them", async () => {
    ...
    const [, call] = await session([INITIALIZE, FIND], answering(problem429(withoutKeys)));
    const text = (call?.result as { content: { text: string }[] }).content[0].text;

    /* The disagreeing control: without it every assertion above passes against a client
       that prints the key sentence unconditionally. */
    expect(text).not.toContain("DARKPRINT_API_KEY");
    expect(text, "the limit is still reported").toContain("2099-01-01T00:00:00.000Z");
  });

  it("still refuses legibly when the 429 carries none of the registry's members", async () => {
    ...
    /* A proxy's own error page reaches a client that is older or newer than the service. A
       formatter that throws here turns a rate limit into an internal error. */
    expect(result.isError).toBe(true);
...
describe("a 404 does not read as an outage, and an outage does not read as a 404", () => {
  it("says the address holds nothing, or is not visible", async () => {
    const [, call] = await session(
      [
        INITIALIZE,
        {
  ...
  it("says the registry is unreachable when the fetch itself fails", async () => {
    ...
    const [, call] = await session([INITIALIZE, FIND], failing);
    const text = (call?.result as { content: { text: string }[] }).content[0].text;
