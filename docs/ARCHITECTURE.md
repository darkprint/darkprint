...
**Last verified against commit `752721d` on 2026-08-19.**
...
| `server/registry/` | **Backend, LIVE.** The read model behind the gallery, the card pages and the ontology facets — `blueprints.ts`, `cards.ts`, `facets.ts`, `joins.ts`, `order.ts`, `scores.ts`, `snapshot.ts`, `actor.ts`, plus `errors.ts`, `store.ts` and `http.ts` from T081. **`withRegistryStore` converts EVERY rejection**, not a classified subset: measured, this read model raises no refusals of its own — no `throw` in any non-test file, and every empty answer is a *value* because B-03 answers 404 over 403 — so a subset would fail **open** on the clause the wrapper exists for. `withRegistryErrors` answers `problem+json` **500** `store-failed` on all eleven routes, `detail` the instance's own `message` byte for byte (D-81-01/02). Merged at `752721d`, tagged `t081-verified` |
