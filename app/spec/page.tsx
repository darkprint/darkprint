 ...
 *
 * Every band on the four `/spec` pages opens with a `.label-lead` and one of these. The
 * sub-sections used to draw at `text-2xl` (24px), which is neither of the two display
 * steps the site has, and carried no mono cue at all — so a page that spends its one
 * cyan `.eyebrow` on the title had further sections a reader could not tell apart while
 * scanning. `.label-lead` is the answer rather than a second eyebrow: the eyebrow names
 * a page or a full-bleed band, and it is rationed to one of each.
 */
const BAND_H2 =
    ...
    <>
      <header className="border-b border-line bg-void py-16 sm:py-20">
        <div className="container-page flex flex-col gap-10">
          ...
          />
          {/* `.prose-lane` rather than `max-w-3xl`: the panel is a question and its
              answer, which is body prose, and at 768px the answer ran past 100
              characters a line. The lead above it keeps the full column — that is the
              author's standing ruling and the reason a deck is 20px here. */}
          <div className="panel prose-lane flex flex-col gap-3 p-5">
      ...
      {/* ---------- the drawing of the stack ----------
          A band of its own, and the first of five. The page used to hold the figure and
          the doors in one `container-page flex flex-col gap-14 py-14`, which separated
          them by 56px of nothing — while `/towards-a-dark-factory`, in the same nav
          group, marks every seam with a full-bleed edge and a ground change. The device
          is the same here, so the two learn sequences read as one system: `border-t`,
          an alternating ground, and the canonical section tier of padding.

          The grounds alternate against `SectionExample` further down, which is a
          full-bleed `bg-void` band this page imports and does not own: the doors below
          take the surface ground so that the seam into the example is a change of
          ground, the way the landing marks one. No border on this first band — the
          header above it closes on its own `border-b`. */}
      <section className="bg-void py-16 sm:py-20">
        {/* No `[&_figcaption]:max-w-[var(--measure)]` wrapper here any more.
            `components/spec/FigureFrame.tsx` carries `.prose-lane` on the figcaption
            itself, which is where the rule belongs and where every page that mounts a
            figure now inherits it. */}
      ...
      >
        <div className="container-page flex flex-col gap-10">
          <div className="flex flex-col gap-3">
            <span className="label-lead">The layers</span>
            <h2 id="layers-heading" className={BAND_H2}>
          ...
          <div className="flex flex-col gap-5">
            ...
            <p className="prose-lane text-[15px] leading-relaxed text-muted">
              ...
              curated core does not have. Every example in this sequence is read
              out of that folder during the build, so a reader copying from here
              is copying a file that loads.
            </p>
      ...
      {/* ---------- what the engine checks, on a bundle in the archive ----------
          Its own full-bleed band already, `bg-void py-20 sm:py-28`, and it carries the
          page's second `.eyebrow` legitimately: the ration is one per page OR per
          full-bleed band. The doors above it are on the alternate ground, so the seam is
          a ground change rather than a rule. */}
      <SectionExample />
      ...
      {/* ---------- the door to how it's graded: not a fourth layer ----------
          ...
          It is below the worked example rather than above it, because it used to close
          the three-layer block and that put the page's last exit before the scorecard
          that argues for taking it.

          `id="scoring"` and `scroll-mt-24` travel with the door: `/spec#scoring` is a
          bookmark `anchors.test.ts` and `spec-routes.test.ts` both hold. Its `border-t`
          and its ground are both changes from `SectionExample` above it. */}
      <section
        className="border-t border-line bg-surface/40 py-16 sm:py-20"
        aria-labelledby="scoring-door-heading"
      >
        <div className="container-page">
          {/* `route-box`, like the three doors above it. `globals.css` states the law in
              its own words — "a box whose job is to send a reader somewhere else wears
              this, and nothing else does" — and names "the three layer doors on `/spec`"
              as users. This is the fourth door and was the one exception, dressed as a
              concept panel. The argument for keeping it framed apart from the other three
              is about *placement*, so the count of layers stays true; it was never an
              argument about hue. */}
          <div
            id="scoring"
            className="route-box scroll-mt-24 flex flex-col gap-3 p-5 sm:flex-row sm:items-center sm:justify-between"
          >
            <div className="flex flex-col gap-1.5">
              <span className="route-label">How it&apos;s graded</span>
              <h2
                id="scoring-door-heading"
                /* The panel tier, 20px, which is a step on the scale — `text-2xl` was
                   24px and is not. It was raised to 24 when the page still held the
                   scorecard, to stop the last exit reading as subordinate to a sub-panel
                   of it; that scorecard is on `/spec/scoring` now, and what this heading
                   has to out-rank is the 18px title on each of the three doors above. A
                   door is a callout, not a section, so it takes the panel step rather
                   than the 28/32 every band header on this page now takes. */
                className="font-display text-xl font-semibold leading-snug tracking-[-0.01em] text-fg"
              >
                {SPEC_SCORING.title}
              </h2>
