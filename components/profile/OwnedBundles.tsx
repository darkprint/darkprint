    ...
    <div className="flex flex-col gap-5 border-b border-line p-5 sm:flex-row sm:items-start">
      <div className="flex min-w-0 flex-1 flex-col gap-2">
        <div className="flex flex-wrap items-center gap-2.5">
          {/* Every row points at the owner's own view of the bundle, published or not.
              A published one also has a public page, and the owner view links to it from
              its aside rather than the list offering two destinations for one name. */}
          <Link
          ...
          <MetaPill tone="surface">{isPrivate ? "Private" : "Public"}</MetaPill>
...
export function OwnedBundles({ rows }: { rows: readonly OwnedRow[] }) {
  ...
  return (
    <section className="overflow-hidden rounded-xl border border-line bg-surface">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-line bg-surface-2 px-5 py-4">
        <h2 className="label-lead">Your blueprints</h2>
      ...
      {rows.map((row) => (
        <Row key={row.bundle.slug} row={row} />
      ))}
      ...
      <div className="flex flex-col gap-4 bg-surface-2/50 px-5 py-4 sm:flex-row sm:gap-5">
        ...
        <div className="flex flex-col gap-2 text-[13px] leading-relaxed text-muted">
          ...
          <p>
            The public rows are read off the archive, digest and all; the star figure beside
            one is seeded community support, because there is no ballot. The private rows
            are seeded whole, in{" "}
            <span className="font-mono text-fg">lib/data/bundles.ts</span>, and nothing
            stores them: they carry a version because a fixture claims one, where a published
            bundle is addressed by the digest of its own bytes.
          </p>
        </div>
      </div>
    </section>
