...
const FROZEN: readonly Frozen[] = [
  {
    ...
    blob: "6b3c120617affd99cf139bb0f2d90e1cd087d8b6" /* RE-PINNED DELIBERATELY AGAIN, hours after the last one, 2026-09-06: the owner deleted the component ("about the Autonomy meter: yes, drop"). `components/ui/AutonomyMeter.tsx` is gone, 200 lines. TWO THINGS MOVED IN THIS FILE AND NEITHER IS A FLOOR BEING SLID. The file-presence premise named that component as the proof the walk was looking in the right place; it names `components/ui/ContentRow.tsx` instead, a surface the surviving rules still govern. And section 2 is RETIRED rather than inverted — it was inverted two hours ago, to exactly-zero call sites, while the component still existed and could return; with the file deleted there is no subject and a cell asserting zero mounts of a deleted file is green in every world. **The component's design argument was carried into this file rather than deleted with it**: doc 2 §1.1 governs how autonomy may be rendered at all, not how one component rendered it, and sections 1 and 3 enforce two halves of it against every surface. FALSIFIED, and the two failures before the success are the useful part. Probe 1 inserted the alarm token before a string that does not occur in `ContentRow.tsx` and never landed. Probe 2 landed in `NodeCardSummary.tsx` but inside a block comment, which `withoutBlockComments` correctly strips — the guard was right and the probe was not. Both returned ZERO reds, which reads exactly like a guard that does not fire. Probe 3 put `text-signal` on the real JSX span at `NodeCardSummary.tsx:388` and the rule red BY NAME, printing `text-signal within 260 chars of the mark`. Restored from a `cp` copy and verified byte-identical each time. The previous pin was `61b3bbecf216fe50e20c073d73a1a4a278802a3a`. */ /* RE-PINNED DELIBERATELY, 2026-09-06, at the owner's instruction to take the Autonomy and static-risk readings off the upload preview ("as we removed that feature"). That was `AutonomyMeter`'s LAST mount, so the cell's floor of one call site became false. **The floor is INVERTED rather than deleted**: the count is asserted at exactly zero, which still fails if the blind scan breaks AND fails if any surface mounts the meter again, so a future mount cannot inherit a green without satisfying the `contributions` rule and re-pinning this file. The rule is untouched and has no subject today. Falsified before the pin moved, on the SECOND attempt, and the first is recorded because it is the more useful half: the probe inserted a mount after `<div className="flex flex-col gap-5">`, a string that does not occur in that file (the real one wraps `cx(...)`), so it never landed and the run came back with ZERO reds. A mutation that fails to apply is indistinguishable from a guard that does not fire, and it reads as the stronger of the two results. Re-run against the real anchor, with the mount confirmed present in the file first, the cell red BY NAME on its own message with 5 of 6 still passing. The file was restored from a `cp` copy and verified byte-identical. The previous pin was `b2114d742d79149f6fd59f843732a4211b277056`. */ /* RE-PINNED DELIBERATELY, 2026-09-06,
