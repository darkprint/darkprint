...
| `/blueprints/[slug]` | `app/blueprints/[slug]/page.tsx:120` | public | One published blueprint: files, graph, cards, scorecard, evidence, history, download, DOT, notes. 9 pages | LIVE + MOCK (metrics `efficacy`/`reliability`/`transparency`/`cost`, `downloads`, `votes`, `comments` from `lib/data/community.ts`; forks/watchers from `lib/data/bundles.ts:172`, `lib/data/profiles.ts:179`) | `ContentCard` tiles on `/blueprints`, `/u/[username]`, `Pinned`, `SavedList`, `/u/[username]/[slug]:300`, `components/bundle/Aside.tsx:190` |
...
| `/nodes/[...id]` | `app/nodes/[...id]/page.tsx:727` | public | One node card at its newest version: spec, interfaces, prohibitions, every field, version history, source. Catch-all so a namespaced id resolves (`:47`). 53 pages | LIVE + MOCK (`downloadsFor`/`starsFor`/`commentsFor`, `lib/data/node-community.ts`) | `NodeCardSummary` tiles on `/nodes`, `Pinned`, `SavedList`, `nodeHref` from graph nodes and `components/bundle/Aside.tsx:203` |
...
| `/u/[username]` | `app/u/[username]/page.tsx:42` | public · owner-view by fixture | Profile overview: pinned, counted archive facts, seeded preview signals, the two grids. 6 pages | LIVE (blueprints, cards, terms counted off `content/`) + MOCK (`profileFor`, `reputation`, `downloads`) | account menu (`SiteHeader.tsx:132`), `AuthorChip` on every card and blueprint, `/settings:173` |
...
| `/u/[username]/blueprints` | `app/u/[username]/blueprints/page.tsx:59` | public · owner-view by fixture | Owner: the management list, public and private together. Visitor: the published shelf. 6 pages | MOCK (owner rows) + LIVE (visitor rows, `:72`) | profile tab strip (`components/profile/tabs.ts:41`), account menu (`SiteHeader.tsx:133`), `/u/[username]:145` |
| `/u/[username]/cards` | `app/u/[username]/cards/page.tsx` | public · owner-view by fixture | Every node card authored under this handle. 6 pages | LIVE | profile tab strip, account menu (`SiteHeader.tsx:134`), `/u/[username]:174` |
