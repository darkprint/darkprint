 ...
 * accounts arrive.
 */
const PROVENANCE = "Generated by the DarkPrint guided path";
...
function approver(p: StarterProfile): StarterCardSpec {
  return {
    ...
    requiresHuman: true,
    riskMarkers: [],
    notes: `With this node the blueprint reads autonomy level 3. Without it, level 4. The level records where a person acts in the run, and this is where. A factory that touches something you cannot roll back is a factory you want this node in, and the number says which of the two designs you are holding. \`requires_human\` is redundant with the node type and set anyway, so the gate is legible without an ontology lookup.`,
    version: BASE_VERSION,
    provenance: PROVENANCE,
    ontologyVersion: ONTOLOGY_VERSION,
  };
