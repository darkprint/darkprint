...
| D-52 | Every bundle carries an agent-facing `AGENTS.md` whose generated half always exists, so nobody is blocked from publishing. | `docs/superpowers/specs/2026-08-04-ia-redesign.md:335-337` ("Approved, 2026-08-04"), `:389-391` | OWNER-STATED (Q1) | REVERSED, 2026-08-25 — `lib/content/bundle-export.ts` no longer writes `AGENTS.md` into any bundle; the constant and its old line numbers (`:68,430`) are gone from the generator. `BUNDLE_AGENTS` stays exported only because two FROZEN test suites (`components/blueprint/download-name.test.ts`, `components/bundle/files.test.ts`) import the name | REVERSED (OWNER-STATED, 2026-08-25 — "remove `factory.dot` and `AGENTS.md` from the blueprint folder entirely. A published blueprint folder must contain only: `topology.dot`, `cards/*.yaml`, `README.md`, and `ontology/extensions.yaml` when the bundle uses local terms."); CONFIRMED from `2026-08-04` until then |
